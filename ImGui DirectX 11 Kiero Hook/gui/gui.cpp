...
void gain_mouse_pos( int& last_x, int& last_y, int& new_x, int& new_y ) {
	POINT p_mouse_pos;
	GetCursorPos( &p_mouse_pos );
	ScreenToClient( FindWindow( 0, "workspace" ), &p_mouse_pos );
	last_x = new_x;
	last_y = new_y;
	new_x = static_cast< int >( p_mouse_pos.x );
	new_y = static_cast< int >( p_mouse_pos.y );

	//std::cout << std::to_string(new_x) + "\n";
	//std::cout << std::to_string(new_y) + "\n";
}
...
void GUI::think( ) {
	...
	bool is_dragging;
	gain_mouse_pos( g_input.m_mouse.x, g_input.m_mouse.y, m_drag_offset.x, m_drag_offset.y );
	//math::clamp( g_input.m_mouse.x, 0, 1920 );
	//math::clamp( g_input.m_mouse.y, 0, 1080 );
	...
	for ( const auto& f : m_forms ) {
		...
		Rect el = f->GetElementsRect( f->m_scroll_pos );
				...
				if ( g_input.IsCursorInRect( cl ) ) {
					...
					if ( g_input.IsCursorInRect( tabs_area ) && !f->m_tabs.empty( ) ) {
						...
						for ( size_t i{}; i < f->m_tabs.size( ); ++i ) {
							...
							Rect tab{ tab_position.left + 5, tab_position.top, tabs_area.w, 16 };
